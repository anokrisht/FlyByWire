...
static DataAcquisitionData acquisition_data;
static uint32_t last_sample_ms;
static uint32_t last_barometer_sample_ms;
static uint32_t last_airspeed_sample_ms;
static uint32_t last_telemetry_ms;

static void update_acquisition_data(uint32_t now_ms)
{
  const Icm20948_Data *imu_data = Imu_GetData(&imu);
  const Imu_Orientation *orientation = Imu_GetOrientation(&imu);
  const Bmp390_Data *barometer_data = Barometer_GetData(&barometer);
  const GpsData *gps_data = GpsService_GetData(&gps);
  const SensorHealth *imu_health = SensorSupervision_GetHealth(
      &sensor_supervision, SUPERVISED_IMU);
  const SensorHealth *barometer_health = SensorSupervision_GetHealth(
      &sensor_supervision, SUPERVISED_BAROMETER);
  const SensorHealth *gps_health = SensorSupervision_GetHealth(
      &sensor_supervision, SUPERVISED_GPS);

  acquisition_data.timestamp_ms = now_ms;
  acquisition_data.imu_valid = imu_data != NULL;
  acquisition_data.orientation_valid = orientation != NULL;
  acquisition_data.barometer_valid = barometer_data != NULL;
  acquisition_data.airspeed_valid = airspeed.data_valid;
  acquisition_data.gps_valid =
      (gps_data != NULL) &&
      SensorSupervision_IsOperational(&sensor_supervision, SUPERVISED_GPS);

  if (imu_data != NULL) acquisition_data.imu = *imu_data;
  if (orientation != NULL) acquisition_data.orientation = *orientation;
  if (barometer_data != NULL)
  {
    acquisition_data.barometer = *barometer_data;
    acquisition_data.altitude_m = Barometer_GetAltitude(&barometer);
  }
  acquisition_data.indicated_airspeed_mps = airspeed.indicated_airspeed_mps;
  acquisition_data.differential_pressure_pa = airspeed.differential_pressure_pa;
  acquisition_data.airspeed_raw_adc = airspeed.raw_adc;
  if (gps_data != NULL) acquisition_data.gps = *gps_data;
  if (imu_health != NULL) acquisition_data.imu_health = *imu_health;
  if (barometer_health != NULL)
  {
    acquisition_data.barometer_health = *barometer_health;
  }
  if (gps_health != NULL) acquisition_data.gps_health = *gps_health;
}
